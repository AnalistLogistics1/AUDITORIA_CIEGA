...
on_('btnDuplicateModeCancel', 'click', closeDuplicateOrdersModal_);
  ...
  on_('btnAdminRefresh', 'click', handleAdminRefresh_);
...
function draftKey_(orden, conteo) {
  var user = (state.session && state.session.usuario) || 'ANON';
  return 'DRAFT_CAPTURE::' + user + '::' + String(orden || '') + '::' + String(conteo || '1');
}

function currentDraftKey_() {
  if (!state.currentAuditorOrder || !state.currentAuditorOrder.header) return '';
  var h = state.currentAuditorOrder.header;
  return draftKey_(h.orden, h.conteoActual || '1');
}

function saveDraftAuditor_() {
  try {
    var key = currentDraftKey_();
    if (!key) return;

    var draft = {
      rows: (state.captureRows || []).map(function (r) {
        return {
          tagIdPaleta: r.tagIdPaleta || '',
          scan: r.scan || '',
          codigo: r.codigo || '',
          cantidad: r.cantidad || '',
          lote: r.lote || '',
          esPriorizado: r.esPriorizado || 'NO',
          resultado: r.resultado || '',
          _lastResolvedScan: r._lastResolvedScan || ''
        };
      }),
      comentario: value_('captureComentarioGlobal') || '',
...
function loadDraftAuditor_(orden, conteo) {
  try {
    var raw = localStorage.getItem(draftKey_(orden, conteo));
    if (!raw) return null;
...
function clearDraftAuditor_(orden, conteo) {
  try {
    localStorage.removeItem(draftKey_(orden, conteo));
  } catch (e) {}
}
...
function renderAuditorOrders_(rows) {
  ...
  if (!rows || !rows.length) {
    tbody.innerHTML = '<tr><td colspan="7">Sin órdenes disponibles.</td></tr>';
    return;
  }
...
var draft = loadDraftAuditor_(res.header.orden, res.header.conteoActual || '1');

if (draft && draft.rows && draft.rows.length) {
  state.captureRows = draft.rows.map(hydrateCaptureRowFromServer_);
  setValue_('captureComentarioGlobal', draft.comentario || '');
} else if (res.captureRowsSuggested && res.captureRowsSuggested.length) {
...
function previewPlantilla_() {
  ...
  leerFilasExcelCliente_(file).then(async function (rows) {
    try {
      if (!rows.length) {
        throw new Error('No se encontraron filas válidas en el Excel.');
      }

      // Guardamos las filas para el guardado posterior
      state.pendingImportPreviewRows = rows;

      var res = await apiPreviewPlantilla(state.session.token, rows);

      state.pendingImportPreviewRows = res.previewRows || rows;
      state.pendingImportDuplicateOrders = res.duplicateOrderDetails || [];
      state.pendingImportTotalRows = res.total || rows.length;
      state.pendingImportDuplicateMode = '';
      ...
      if (state.pendingImportDuplicateOrders.length > 0) {
        ...
        setMessage_('uploadMessage', 'Vista previa generada. No se detectaron órdenes duplicadas.', 'success');
      }
    } catch (err) {
      hideAppModal_();
      resetImportState_();
    ...
    hideAppModal_();
    resetImportState_();
    setMessage_('uploadMessage', err.message || 'Error leyendo el archivo Excel.', 'error');
  });
}

function resetImportState_() {
  state.pendingImportTempId = '';
  state.pendingImportPreviewRows = [];
  state.pendingImportDuplicateOrders = [];
  state.pendingImportTotalRows = 0;
...
// Lee el Excel con SheetJS y devuelve las filas listas (como hacía el backend antes)
function leerFilasExcelCliente_(file) {
  return new Promise(function (resolve, reject) {
    ...
    reader.onload = function (e) {
      try {
        ...
        // Matriz de valores como texto (equivale a getDisplayValues)
        var matrix = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false, defval: '' });

        var rows = matrixToTemplateRowsCliente_(matrix);
        resolve(rows);
      } catch (err) {
        reject(err);
      }
...
// Traducción exacta de matrixToTemplateRowsServer_ (misma lógica y validaciones)
function matrixToTemplateRowsCliente_(matrix) {
  if (!Array.isArray(matrix) || !matrix.length) return [];

  var startIndex = 0;
  var firstRow = matrix[0] || [];

  if (normalizeText_(firstRow[0]) === 'GRUPO' && normalizeText_(firstRow[1]) === 'ORDEN') {
    startIndex = 1;
  }
  ...
  for (var i = startIndex; i < matrix.length; i++) {
    var c = matrix[i] || [];
    var excelRowNumber = i + 1;

    var grupo = String(c[0] || '').trim();
    var orden = String(c[1] || '').trim();
    var sku = String(c[2] || '').trim();

    if (!orden || !sku) continue;

    var cantidadBase = parseNumberCliente_(c[6] || 0);
    var valorEan14 = validarValorEan14Cliente_(c[11], excelRowNumber);

    rows.push({
      grupo: grupo,
      orden: orden,
      sku: sku,
      lote: String(c[3] || '').trim(),
      fv: String(c[4] || '').trim(),
      descripcion: String(c[5] || '').trim(),
      cantidad: cantidadBase,
      ean13: String(c[7] || '').trim(),
      inner: String(c[8] || '').trim(),
      valorInner: parseNumberCliente_(c[9] || 0),
      ean14: String(c[10] || '').trim(),
      valorEan14: valorEan14,
      cantidadEan14Preview: cantidadBase / valorEan14,
      cantidadEan13SkuPreview: cantidadBase
    });
  }
...
function parseNumberCliente_(value) {
  var n = parseFloat(String(value == null ? '' : value).replace(',', '.'));
  return isNaN(n) ? 0 : n;
}

function validarValorEan14Cliente_(rawValue, excelRowNumber) {
  var raw = String(rawValue == null ? '' : rawValue).trim();
  if (!raw) {
    throw new Error('La columna L (Valor EAN14) está vacía en la fila ' + excelRowNumber + '. Corrija el archivo y vuelva a cargarlo.');
  }
  var value = parseNumberCliente_(raw);
  if (value <= 0) {
    throw new Error('La columna L (Valor EAN14) es inválida en la fila ' + excelRowNumber + '. Debe ser mayor que 0.');
  }
  return value;
}
...
async function guardarPlantillaPendiente_() {
  if (!state.pendingImportPreviewRows || !state.pendingImportPreviewRows.length) {
  ...
  // Reconstruir las filas originales para enviar al backend
  var rowsParaEnviar = state.pendingImportPreviewRows.map(function (r) {
    return {
      grupo: r.grupo, orden: r.orden, sku: r.sku, lote: r.lote, fv: r.fv,
      descripcion: r.descripcion, cantidad: r.cantidad, ean13: r.ean13,
      inner: r.inner, valorInner: r.valorInner, ean14: r.ean14, valorEan14: r.valorEan14
    };
  });
  ...
  try {
    ...
    var res = await apiImportarPlantilla(
      state.session.token,
      rowsParaEnviar,
      state.pendingImportDuplicateMode || 'APPEND'
    );
...
async function saveCaptureRows_() {
  ...
  // Preparar las filas para el envío en lote
  var rowsPayload = rows.map(function (r) {
    return {
      tagIdPaleta: r.tagIdPaleta || '',
      scan: r.scan || '',
      cantidadAuditada: Number(r.cantidad || 1),
      loteAuditado: r.lote || '',
  ...
  try {
    ...
    // UNA SOLA petición con todas las filas (antes era un for con N peticiones)
    await apiRegistrarAuditoriaLote(
      state.session.token,
      state.currentAuditorOrder.header.orden,
      state.currentAuditorOrder.header.conteoActual || '1',
      rowsPayload
    );

    // Cerrar el conteo (igual que antes)
...
async function handleDescargarRegistroVerificacion_() {
  ...
  try {
    ...
    var res = await apiDataRegistroVerificacion(state.session.token);
    var dataRows = res.rows || [];

    // ===== FILA 1: categorías agrupadas (40 columnas A-AN) =====
    var row1 = [
      'Datos de la Orden','','','','','','','','','','','','','',  // A-N (14)
      'Resultado Conteo','',                                        // O-P (2)
      '1er Conteo','','','','','','',                               // Q-W (7)
      '2do Conteo','','','','','','',                               // X-AD (7)
      '3er Conteo','','','','','','',                               // AE-AK (7)
      'Actualización','',''                                         // AL-AN (3)
    ];

    // ===== FILA 2: nombres de columna =====
    var row2 = [
      'ID','Código de Carga','Grupo','Orden','Estado','Codigo','Lote','Ean13','Inner','Valor','Ean14','Valor','Descripción','Cantidad',
      'Cantidad','Conteos',
      'Cantidad','Ean13','Inner','Ean14','Fecha','Auditor','Comentario',
      'Cantidad','Ean13','Inner','Ean14','Fecha','Auditor','Comentario',
      'Cantidad','Ean13','Inner','Ean14','Fecha','Auditor','Comentario',
      'Fecha','Responsable','Comentario'
    ];

    // ===== Armar la hoja: fila1 + fila2 + datos (desde fila 3) =====
    var aoa = [row1, row2].concat(dataRows);
    var ws = XLSX.utils.aoa_to_sheet(aoa);

    // ===== Combinaciones de celdas (merges) para la fila 1 =====
    ws['!merges'] = [
      { s: { r: 0, c: 0 },  e: { r: 0, c: 13 } }, // Datos de la Orden (A-N)
      { s: { r: 0, c: 14 }, e: { r: 0, c: 15 } }, // Resultado Conteo (O-P)
      { s: { r: 0, c: 16 }, e: { r: 0, c: 22 } }, // 1er Conteo (Q-W)
      { s: { r: 0, c: 23 }, e: { r: 0, c: 29 } }, // 2do Conteo (X-AD)
      { s: { r: 0, c: 30 }, e: { r: 0, c: 36 } }, // 3er Conteo (AE-AK)
      { s: { r: 0, c: 37 }, e: { r: 0, c: 39 } }  // Actualización (AL-AN)
    ];

    var wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Registro_Verificacion');

    // ===== Nombre del archivo con fecha/hora =====
    var ahora = new Date();
    var p = function (n) { return String(n).padStart(2, '0'); };
    var nombre = 'Registro_Verificacion_' +
      ahora.getFullYear() + p(ahora.getMonth() + 1) + p(ahora.getDate()) + '_' +
      p(ahora.getHours()) + p(ahora.getMinutes()) + p(ahora.getSeconds()) + '.xlsx';
...
async function handleAdminRefresh_() {
  try {
    showAppModal_('ACTUALIZANDO REGISTROS...', true);
    await loadAdminOrders_();
  } catch (err) {
    alert(err.message || 'Error actualizando la vista Admin.');
  } finally {
    hideAppModal_();
  }
}

async function handleAuditorRefresh_() {
  try {
    showAppModal_('ACTUALIZANDO REGISTROS...', true);
    await loadAuditorOrders_();
  } catch (err) {
    alert(err.message || 'Error actualizando la vista Auditor.');
  } finally {
    hideAppModal_();
  }
}
